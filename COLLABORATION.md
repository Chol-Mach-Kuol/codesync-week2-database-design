...
> **YOUR TASKS:**
> 1. 🗄️ Create `database/database_setup.sql`
> 2. 💾 Create `examples/json_schemas.json`
> 3. 🔀 Open a Pull Request to main
>
> **Deadline: Tuesday Sep 16 at 11:59pm**
...
```bash
git clone https://github.com/Chol-Mach-Kuol/codesync-week2-database-design.git
cd codesync-week2-database-design
...
### Step 3 — Create `database/database_setup.sql`

Run this command to open the file:
```bash
nano database/database_setup.sql
```
Paste the content below, then press **Control+X → Y → Enter** to save:
...
INSERT INTO transactions (transaction_id, category_id, user_id, amount, fee, balance_after, transaction_date, raw_body, status) VALUES
    ('TXN-2024-0001', 1, 1, 50000.00,    0.00, 120000.00, '2024-08-01 09:15:00', 'You have received 50,000 RWF from Alice Uwimana (+250781000001). Your new balance is 120,000 RWF.', 'success'),
    ('TXN-2024-0002', 3, 2, 20000.00,  200.00,  99800.00, '2024-08-05 14:30:00', 'TXN-2024-0002: Transfer of 20,000 RWF to Bob Nkurunziza (+250782000002) successful. Fee: 200 RWF. Balance: 99,800 RWF.', 'success'),
    ('TXN-2024-0003', 5, 3,  1000.00,    0.00,  98800.00, '2024-08-10 08:00:00', 'Airtime payment of 1,000 RWF to Claire Mukamana (+250783000003). Balance: 98,800 RWF.', 'success'),
    ('TXN-2024-0004', 8, 4, 30000.00,  300.00,  68500.00, '2024-08-15 11:45:00', 'Cash withdrawal of 30,000 RWF at agent David Habimana (+250784000004). Fee: 300 RWF. Balance: 68,500 RWF.', 'success'),
    ('TXN-2024-0005', 2, 5,  5000.00,   50.00,  63450.00, '2024-08-20 16:00:00', 'Payment of 5,000 RWF to code holder Eve Ingabire (+250785000005). Fee: 50 RWF. Balance: 63,450 RWF.', 'success');

INSERT INTO tags (name) VALUES
    ('high-value'), ('flagged'), ('reviewed'), ('duplicate-check'), ('reconciled');
...
SELECT c.name, COUNT(*) AS tx_count, SUM(t.amount) AS total_amount, SUM(t.fee) AS total_fees
FROM transactions t JOIN categories c ON t.category_id = c.id
GROUP BY c.name ORDER BY total_amount DESC;

SELECT u.phone_number, u.name, COUNT(*) AS tx_count, SUM(t.amount) AS total_amount
FROM transactions t JOIN users u ON t.user_id = u.id
GROUP BY u.id ORDER BY total_amount DESC LIMIT 5;

UPDATE transactions SET status = 'reversed' WHERE transaction_id = 'TXN-2024-0003';
...
### Step 4 — Create `examples/json_schemas.json`

Run this command to open the file:
```bash
nano examples/json_schemas.json
```
Paste the content below, then press **Control+X → Y → Enter** to save:

```json
{
  "_comment": "CodeSync — MoMo SMS Analytics Platform | JSON Schemas | Week 2",
  "entities": {
    "category": { "id": 1, "name": "incoming_money", "description": "Money received from another MoMo user" },
    "user": { "id": 1, "phone_number": "+250781000001", "name": "Alice Uwimana", "first_seen": "2024-01-05", "last_seen": "2024-08-20" },
    "transaction": { "id": 1, "transaction_id": "TXN-2024-0001", "category_id": 1, "user_id": 1, "amount": 50000.00, "fee": 0.00, "balance_after": 120000.00, "transaction_date": "2024-08-01T09:15:00", "raw_body": "You have received 50,000 RWF from Alice Uwimana (+250781000001). Your new balance is 120,000 RWF.", "status": "success" },
    "tag": { "id": 1, "name": "high-value" },
    "system_log": { "id": 1, "transaction_id": "TXN-2024-0001", "level": "INFO", "message": "Transaction TXN-2024-0001 parsed and inserted successfully.", "created_at": "2024-08-01T09:15:05" }
  },
  "complex_transaction": {
    "id": 1, "transaction_id": "TXN-2024-0001", "amount": 50000.00, "fee": 0.00, "balance_after": 120000.00,
    "transaction_date": "2024-08-01T09:15:00", "status": "success",
    "raw_body": "You have received 50,000 RWF from Alice Uwimana (+250781000001). Your new balance is 120,000 RWF.",
    "category": { "id": 1, "name": "incoming_money", "description": "Money received from another MoMo user" },
    "user": { "id": 1, "phone_number": "+250781000001", "name": "Alice Uwimana", "first_seen": "2024-01-05", "last_seen": "2024-08-20" },
    "tags": [ { "id": 1, "name": "high-value" }, { "id": 3, "name": "reviewed" } ],
    "logs": [ { "id": 1, "level": "INFO", "message": "Transaction TXN-2024-0001 parsed and inserted successfully.", "created_at": "2024-08-01T09:15:05" } ]
  },
  "sql_to_json_mapping": {
    "categories.id": "category.id", "categories.name": "category.name", "categories.description": "category.description",
    "users.id": "user.id", "users.phone_number": "user.phone_number", "users.name": "user.name",
    "transactions.id": "transaction.id", "transactions.category_id": "transaction.category (nested object)",
    "transactions.user_id": "transaction.user (nested object)", "transaction_tags": "transaction.tags (nested array)",
...
```bash
git add .
git commit -m "feat: add database_setup.sql and json_schemas"
git push origin feature/database-api
```

### Step 6 — Open a Pull Request

1. Go to `https://github.com/Chol-Mach-Kuol/codesync-week2-database-design`
2. Click **Compare & pull request**
3. Set base: `main` ← compare: `feature/database-api`
